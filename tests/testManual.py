...
class TestManual(unittest.TestCase):
    ...
    def testVolumeList(self):
        ...
        self.assertIsNotNone(self._manage_obj.volume_list())
    ...
    def testInstanceList(self):
        ...
        self.assertIsNotNone(self._manage_obj.instance_list())

    def __search_volume__(self, vol_id):
        """
            search volume in full list, only for check list
        """
        for volume in self._manage_obj.volume_list():
            if volume['id'] == vol_id:
                return volume
        return None
    ...
    def __create_volume__(self):
        ...
        # create something
        created = self._manage_obj.volume_create(size, new_name, desc)
        ...
        # directly get
        volume = self._manage_obj.volume_get(created['id'])
        self.assertIsNotNone(volume)
        self.assertEqual(volume['name'], new_name)
        self.assertEqual(volume['description'], desc)
        self.assertEqual(volume['size'], size)
        ...
        # get by name
        volume = self._manage_obj.volume_get(name=new_name)
        self.assertIsNotNone(volume)
        self.assertEqual(volume['name'], new_name)
        self.assertEqual(volume['description'], desc)
        self.assertEqual(volume['size'], size)
    ...
    def __drop_volume__(self, created):
        """
            drop some volume
        """
        self._manage_obj.volume_delete(created['id'])
        volume = self.__search_volume__(created['id'])
        # must be deleting
    ...
    def testGetException(self):
        ...
        with self.assertRaises(ManageExeption):
            self._manage_obj.volume_get(some_id)
        with self.assertRaises(ManageExeption):
            self._manage_obj.volume_get(name=some_id)
    ...
    def testDropByName(self):
        ...
        created = self.__create_volume__()
        self._manage_obj.volume_delete(name=created['name'])
        volume = self.__search_volume__(created['id'])
    ...
    def testFullSequnce(self):
        ...
        self.assertIsNotNone(self._manage_obj.volume_list())
        created = self.__create_volume__()
        instances = self._manage_obj.instance_list()
        ...
        # self._manage_obj.volume_format(
        #    "/dev/vdn", open(
        ...
        # detach
        self._manage_obj.volume_detach(created['id'])
